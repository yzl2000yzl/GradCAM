...
def prepare_input(image):
    ...
    image -= means
    image /= stds
    image = np.ascontiguousarray(np.transpose(image, (2, 0, 1)))  # (224, 224, 3)->(3, 224, 224)
    image = image[np.newaxis, ...]  # 在原来维度上加1，(3, 224, 224)->(1, 3, 224, 224)
...
def main(args):
    ...
    img = np.float32(cv2.resize(img, (224, 224))) / 255 # 读入图像, 0-255化为0-1
    print("image size:",img.shape)
    inputs = prepare_input(img)# 图像归一化
    print("input size:",inputs.shape)
    # 输出图像
    image_dict = {}
    # 网络
    net = get_net(args.network, args.weight_path)# 根据网络名称获取模型
    # Grad-CAM
